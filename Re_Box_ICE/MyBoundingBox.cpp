...
MyBoundingBoxClass::MyBoundingBoxClass(std::vector<vector3> vertexList)
{
	...
	for (int i = 1; i < reVertexList.size(); i++) {
		...
		{
			m_ReMin.x = reVertexList[i].x;
		}
		else if (m_ReMax.x < reVertexList[i].x)
		{
			m_ReMax.x = reVertexList[i].x;
		}

		if (m_ReMin.y > reVertexList[i].y)
		{
			m_ReMin.y = reVertexList[i].y;
		}
		else if (m_ReMax.y < reVertexList[i].y)
		{
			m_ReMax.y = reVertexList[i].y;
		}

		if (m_ReMin.z > reVertexList[i].z)
		{
			m_ReMin.z = reVertexList[i].z;
		}
		else if (m_ReMax.z < reVertexList[i].z)
		{
			m_ReMax.z = reVertexList[i].z;
		}
	}

	reCenter = m_v3CenterGlobal = (m_ReMax + m_ReMin) / 2.0f;

	reSize.x = glm::distance(vector3(m_ReMin.x, 0.0, 0.0), vector3(m_ReMax.x, 0.0, 0.0));
	reSize.y = glm::distance(vector3(0.0, m_ReMin.y, 0.0), vector3(0.0, m_ReMax.y, 0.0));
	reSize.z = glm::distance(vector3(0.0, 0.0, m_ReMin.z), vector3(0.0, 0.0, m_ReMax.z));
